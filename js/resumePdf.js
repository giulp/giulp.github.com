...
function findSymbolForClass(selector) {
  ...
  for (var sheetNr = 0; sheetNr < sheets.length; sheetNr++) {
    var content = this.findCSSRuleContent(sheets[sheetNr], selector);
...
function ResumePdf(resumeData, lang){  
  pdfMake.fonts  = {
    exo: {
  ...
  this.starsString = function(stars){
    var i = j = 0, ret = [], empty = 5 - stars;
    if(stars == undefined) return ' ';
    while(i < stars){ ret.push(ICO_STAR); i++; }
    while(j < empty){ ret.push(ICO_STAR_0); i++; }
    return ret;
  };
  this.open = function(){ pdfMake.createPdf(docDefinition).open(); };
        ...
        table: {
          ...
          body: [
            [                   
              ...
              { stack: [
                { text: this._data[this.lang].general.pdf_title, style: 'headerTitle', border: ALLFALSE },
  ...
  this._dev        = function(){
    var elenco = [{ text: [ICO_GROUPWORK, this._data[this.lang].general.main_dev], margin: [0, 0, 0, 5] }]
      .concat(this._data[this.lang].resume.experience.dev.map(function(el, i){
        return { table: { widths: ['*', 'auto', '1'], border: ALLFALSE,
          body: [
            ...
            [{ul: el.achievements, colSpan: 2, border: ALLFALSE, style: 'devDescr' }, LASTFAUX]
          ]}};
      }));
    return { table: { widths: ['*'], margin: [0,5], border: ALLFALSE,
        body: [
          [{ text: this._data[this.lang].general.menu_dev, style: 'devCoaching', border: BOTTOMONLY }],
          [{ table:{ widths: ['*'], body: [[ { stack: elenco, border: ALLFALSE } ]], border: ALLFALSE }, border: ALLFALSE }]
        ]}};
  };
  this._coaching   = function(){
    var elenco = [{ text: [ICO_GROUPWORK, this._data[this.lang].general.main_dev], margin: [0, 0, 0, 5] }]
      .concat(this._data[this.lang].resume.experience.mentoring.map(function(el, i){
    ...
    return { table: { widths: ['*'], body: [
          [{ text: this._data[this.lang].general.menu_coaching, style: 'devCoaching', border: BOTTOMONLY }],
  ...
  this._skills     = function(){
    var self = this;
    return [{ text: [ICO_KEY, ' ' + this._data[this.lang].general.section_skill] }].concat(this._data[this.lang].resume.skills.map(function(el, i){
      var stars = self.starsString(el.stars);
      return {
        table: {
          widths: ['*', 'auto', '1'],
          body: [FAUXROW,
  ...
  this._langs      = function(){
    var self = this;
    return [
      { text: [ICO_GLOBE, ' ' + this._data[this.lang].general.section_langs] },
      ' ',
      {
        table: {
          widths: ['*','*','*','*','*','*','1'],
          body: 
          [
            this._data[this.lang].resume.langs.map(function(el, i){ 
              return { stack: [
              {text: el.title, bold: true, style: 'langName', margin: [10,0,0,0]},
              {text: self.starsString(el.stars), style:{alignment: 'center'}}
  ...
  this._progetti   = function(){
    return [{ text: [ICO_COLLPLUS, this._data[this.lang].general.h1_projects] }].concat(this._data[this.lang].resume.projects.map(function(el, i){
      return {
        table: {
          widths: ['*', 'auto', '1'],
          body: [FAUXROW,
  ...
  this._extra      = function(){
    return [{ text: [ICO_HOME, ' Extra'] }].concat(this._data[this.lang].resume.extracurriculars.map(function(el, i){
      return {
        table: {
          widths: ['auto', '*', '1'],
          body: [FAUXROW,
  ...
  this.docDefinition = {
    ...
    footer: this._footer,
    content: [ 
      this._dev(), this._coaching(),
      { border: ALLFALSE, pageBreak: 'before', table:{ widths: ['*'],
          body: [
            [{ stack: this._skills(),    border: [false,true,false,false], margin: [0, 3, 0, 6] }],
            [{ stack: this._langs(),     border: [false,true,false,false], margin: [0, 3, 0, 6] }],
            [{ stack: this._progetti(),  border: [false,true,false,false], margin: [0, 3, 0, 6] }],
            [{ stack: this._extra(),     border: [false,true,false,false], margin: [0, 3, 0, 6] }]
          ], border: ALLFALSE
        }}],
      images: this.images
  };
